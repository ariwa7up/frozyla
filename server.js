...
app.get("/api/menu", async (req, res) => {
  try {
    ...
    // If no items in DB, return mock data
...
/*if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`🚀 Frozyla API running on http://localhost:${PORT}`);
    console.log(`📦 Environment: ${process.env.NODE_ENV || "development"}`);
    console.log(
      `🔑 Auth: ${process.env.JWT_SECRET ? "Configured" : "⚠️  JWT_SECRET not set"}`,
    );
    console.log(
      `📊 Supabase: ${process.env.SUPABASE_URL ? "Connected" : "⚠️  SUPABASE_URL not set"}`,
    );
  });
}*/
